/// For each data point, compute Moon fraction at Sringeri amrita time.
/// Groups by nakshatra and shows fraction distribution across months.
/// Key question: is the fraction consistent within same nk across months?
///
/// Run: dart run bin/diagnose_frac_all.dart
...
class E {
  final String lbl;
  final DateTime date;
  final int nk; // Sringeri's nakshatra
  final int v;  // weekday 0=Sun..6=Sat
  final int off;
...
void main() {
  final Map<int, List<(String, double, double)>> byNk = {};
  for (int i = 1; i <= 27; i++) byNk[i] = [];
  ...
  for (final e in entries) {
    ...
    final double jd = JulianDay.fromIST(amrita);
    final double lon = moonSidLon(jd);
    final int drikNk = (lon / nkSpan).floor() % 27 + 1; // 1-based
    final double frac = (lon % nkSpan) / nkSpan;
    final double spd = moonSpeed(jd);
    final String match = drikNk == e.nk ? '✓' : '✗(Drik=${drikNk})';

    byNk[e.nk]!.add((e.lbl, frac, spd));
    print('${e.lbl.padRight(22)} S${e.nk.toString().padLeft(2)} $match  '
        'frac=${frac.toStringAsFixed(3)}  spd=${spd.toStringAsFixed(2)}°/d');
  }

  print('');
  print('═' * 70);
  print('Fraction summary by Sringeri nakshatra:');
  print('─' * 70);
  for (int nk = 1; nk <= 27; nk++) {
    final pts = byNk[nk]!;
    if (pts.isEmpty) continue;
    final fracs = pts.map((p) => p.$2).toList()..sort();
    final mean = fracs.reduce((a, b) => a + b) / fracs.length;
    final mn = fracs.first, mx = fracs.last;
    final spread = mx - mn;
    // spread in minutes at mean speed
    final avgSpd = pts.map((p) => p.$3).reduce((a, b) => a + b) / pts.length;
    final spreadMin = (spread * nkSpan / (avgSpd / 1440)).round();
    final status = spreadMin <= 30 ? '✓' : (spreadMin <= 60 ? 'WARN' : '✗');
    print('NK${nk.toString().padLeft(2)} ${nkNames[nk].padRight(18)}: N=${pts.length.toString().padLeft(2)}  '
        'mean=${mean.toStringAsFixed(3)}  spread=${spread.toStringAsFixed(3)} '
        '(${spreadMin}min)  [${'${mn.toStringAsFixed(3)}'}-${'${mx.toStringAsFixed(3)}'}]  $status');
  }
}
